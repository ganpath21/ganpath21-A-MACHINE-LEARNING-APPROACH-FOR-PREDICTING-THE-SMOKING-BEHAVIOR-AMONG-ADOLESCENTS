...
path_df = os.path.join(root, 'recons_dataset/dataset4.csv')
data = pd.read_csv(path_df)

scaler = MinMaxScaler()
train, test = train_test_split(data, test_size=0.10)

X_train = train.drop('Q43', axis=1)
Y_train = train['Q43']

X_test = test.drop('Q43', axis=1)
Y_test = test['Q43']
...
model_path = os.path.join(root, 'models/rfc4.sav')
joblib.dump(clf1, model_path)

# Saving the scaler object
scaler_path = os.path.join(root, 'models/scaler4.pkl')
