...
path_df = os.path.join(root, 'recons_dataset/dataset2.csv')
data = pd.read_csv(path_df)

scaler = MinMaxScaler()
train, test = train_test_split(data, test_size=0.10)

X_train = train.drop('Q16', axis=1)
Y_train = train['Q16']

X_test = test.drop('Q16', axis=1)
Y_test = test['Q16']
...
model_path = os.path.join(root, 'models/rfc2.sav')
joblib.dump(clf1, model_path)

# Saving the scaler object
scaler_path = os.path.join(root, 'models/scaler2.pkl')
