...
path_df = os.path.join(root, 'recons_dataset/dataset3.csv')
data = pd.read_csv(path_df)

scaler = MinMaxScaler()
train, test = train_test_split(data, test_size=0.10)

X_train = train.drop('Q17', axis=1)
Y_train = train['Q17']

X_test = test.drop('Q17', axis=1)
Y_test = test['Q17']
...
model_path = os.path.join(root, 'models/rfc3.sav')
joblib.dump(clf1, model_path)

# Saving the scaler object
scaler_path = os.path.join(root, 'models/scaler3.pkl')
